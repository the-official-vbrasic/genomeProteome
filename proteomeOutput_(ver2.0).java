...
class proteomeOutput {
   ...
   public static void main (String[] args) throws java.lang.Exception {
      ...
      while (loopCheck) {
         System.out.println("Please enter the string of codons (DNA/mRNA) to be sequenced. (No spaces.)");  
         DNACode = userInput.nextLine(); // Gets mRNA code.
         DNACode = DNACode.replaceAll("\\s+",""); // Regex to get rid of spaces. So any non-multiples of 3 problems due to spacing is dealt with.
         DNACode = DNACode.replaceAll("[0-9]",""); // Regex to get rid of numbers.
         DNACode = DNACode.replaceAll("[\\W+]",""); // Regex for all special chars.
         ...
         if (DNACode.length()%3 !=0){ // Codons are triplets. Throws codes which don't have a multiple of three chars.
            ...
            DNACode = DNACode.toUpperCase(); // Makes sure code is uppercase. So gets rid of the need for exception handling here.
            DNACode = DNACode.replaceAll("u", "T"); // Replaces u with T. mRNA can be entered as well.
            DNACode = DNACode.replaceAll("U", "T"); // Replaces U with T. Same as above line.
            ...
            if (counterChecker == 0) {
               System.out.println("Codons entered could not be matched in the database. Please make sure you have entered valid DNA/mRNA codons."); 
